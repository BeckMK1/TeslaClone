<template>
	<div @click="setCurrentMobile" @mouseenter="setCurrent" class="textLinkContainer" :class="isMain == true ? 'mainTextLinkContainer': ''">
		<NuxtLink class="navLink" :to="link"><p class="linkCta" :class="isMain == true && store.isnavDrop == false ? store.menuColor : ''" v-if="isIcon == false">{{ title }}</p> <font-awesome-icon class="linkCta icon" :class="isMain == true && store.isnavDrop == false ? store.menuColor : ''"  v-if="isIcon == true" :icon="icon" /></NuxtLink>
